...
document.ModelDOM_classes.ModelDOM_misc = ModelDOM_misc = class ModelDOM_misc {
  ...
  periodicTasks() {
    ...
    if (body.hasClass("has-scrollbar")) {
      if (document.body.scrollHeight <= document.body.clientHeight) {
        return body.removeClass("has-scrollbar");
      }
    } else {
      if (document.body.scrollHeight > document.body.clientHeight) {
        return body.addClass("has-scrollbar");
      }
    }
  }
  ...
  templateForModel(model, subtemplatePath) {
    ...
    if (subtemplatePath) {
      let templateId;
      if (
        model.fields.subtemplates &&
        (templateId = model.fields.subtemplates[subtemplatePath]) &&
        (ret = this.model(templateId))
      ) {
        return ret;
      }
    } else {
      if (
        ...
        model.fields.template.array &&
        model.fields.template.array.length >= 1 &&
        (ret = model.fields.template.array[0].model)
      ) {
        return ret;
      }
    }
  ...
  changePage(newId, left, top) {
    let jqel;
    if (this._doDebugCall) {
      return this.debugCall("changePage", ["newId"], arguments);
    } else {
      if ((this._doDebugCall = this.doDebugCall)) {
        console.log.apply(this, this._debugCallArgs);
        window.BP();
      }
    }
