...
document.ModelDOM_classes.ModelDOM_server = (ModelDOM_server = class ModelDOM_server {
  constructor() {
    ...
    const isLocal = (window.location.hostname==='localhost') || (window.location.hostname==='127.0.0.1') || window.location.hostname.endsWith('.local') || /192.168.1.\d{1,3}/.test(window.location.hostname);
  ...
  latestAllModelsBody(asObj){
    ...
    if (!asObj) { body = JSON.stringify(body); }
    return body;
  }
  ...
  latestRootModelBody(asObj, resetVersions){
    //if @_doDebugCall then return @debugCall("latestAllModelsBody",["asObj"],arguments) else (if @_doDebugCall = @doDebugCall then console.log.apply(this,@_debugCallArgs); window.BP())
    let models, root;
    if ((!(root=this.models.root)) || !root.fields.page || !(models=root.fields.page.array).length) { return; }
    let body = {
      subscribe:{}
    };
  ...
  latestSubscriptionBody(asObj){
    ...
    if (!Object.keys(body.subscribe).length && !body.unsubscribe.length) { return; }
  ...
  processLatest(data){
    ...
    if (data) {
      for (let id in data) {
        var model;
        const o = data[id];
        if (!(model=this.models[id])) { continue; }
  ...
  submitWSMessage(event, message, modelId, formSelOrData){
    ...
    if ($.isPlainObject(formSelOrData)) {
      ...
      let jqform;
      if ((jqform=$(formSelOrData)).length !== 1) { return; }
      data.form = this.formAsNameValues(jqform[0]);
    }

    if (data.modelMessage && !data.message) { data.message = data.modelMessage; }
  ...
  open() {
    const me = this;
    if (!WebSocket || (this._isOpen!==undefined)) { return; }
  ...
  _open() {
    ...
    ws.onmessage = function(evt){
      ...
      if (match = msg.match(/^(\d+):/)) {
        ...
        if (!(cb = me.callbackOfType(type))) { return; }
        if (typeof(cb)==='function') { cb(msg); }
      }
    };
    ...
    return ws.onclose = function() {
      ...
      if (me._NconnectFail >= 5) {
        ...
        return setTimeout(() => me.sendAllModels()
        ,20000);
      } else {
        return setTimeout(() => me.sendAllModels()
        ,4000);
  ...
  callbackOfType(type){
    const me = this;
    switch (type) {
      case 'Models': return dataString=> me.processLatest(JSON.parse(dataString));
      case 'Changed': return () => me.sendAllModels();
      case 'Phoenix': return function(dataString){
  ...
  sendRootModel(resetVersions){
    let body;
    if (!(body = this.latestRootModelBody(undefined,resetVersions))) { return; }
  ...
  sendModelsNow() {
    ...
    if (!(body = this.latestSubscriptionBody())) { return; }
