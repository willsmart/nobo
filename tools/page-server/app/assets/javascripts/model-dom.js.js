...
document.ModelDOM_classes.ModelDOM_dom = (ModelDOM_dom = (function() {
  ModelDOM_dom = class ModelDOM_dom {
    ...
    refreshLocation(ifChanged){
      ...
      if ((!(model = this.models['root'])) || (!(a = model.fields['page'].array)) || !a.length || (!(page = a[0].model))) { return; }

      if (ifChanged) {
        if ((!this.changedModels['root'] || !model.fieldChanges['page']) && (!this.changedModels[page.id] || !page.fieldChanges['name'])) { return; }
      }
      ...
      if (!(location=this.constructLocation(page))) { return; }
  ...
  //          @stoppedUsingEndpoint(ep)
    ...
    updateModels() {
      if (!this.updateIndex) { this.updateIndex = 1; }
    ...
    updateTextAndAttributesOfModels() {
      ...
      const me = this;
      $(sels).filter(':not(.__deadModel)').each(function() {
    ...
    updateTextAndAttributesOfElement(el,model){
      ...
      if (modelEl.hasClass('__deadModel')) { return; }
      ...
      for (let node of el.childNodes) {
        if (node.nodeType === 3) { // text node
          ...
          if (template = el.getAttribute(`__template_textNode-${textNodeIndex}`)) {
            ...
            if ((typeof(s)==='string') && (s!==node.textContent)) { node.textContent = s; }
          }
          textNodeIndex++;
        }
      }
        ...
        for (let field in model.fieldChanges) {
          ...
          if ($.isPlainObject(f = model.fields[field]) && f.array && f.changes) {
            ...
            for (let index in f.changes) {
              ...
              if ((change.type==='insert') || (change.type==='edit')) {var newMappings;
    ...
    remakeModelElement(el){
      ...
      if (((jqel = $(`#${el.id}`)).length!==1) || (jqel[0]!==el)) { return; }

      if (!(jqwrapper=$(`#${el.id}_base`)).length) { jqwrapper = jqel; }
      ...
      if ((!(model = this.models[el.getAttribute('itemmodelid')])) || (!(parentModel = this.models[el.getAttribute('collectionmodelid')]))) { return; }

      if (el.hasAttribute('overridemodelid')) {
        if (!(overrideModel = this.models[el.getAttribute('overridemodelid')])) { return; }
      }
    ...
    insertNewModelElement(parentModel,jqprevEl,collectionIndex,jqparentEl,model,overrideModel,index,field,markerClass,justReturnMapping,depth,inSubtemplateForModelIds){
      ...
      if (!markerClass) { markerClass = parentModel.classAsChild+"__"+this.markerIndex(field); }
      ...
      for (let overrideCount = 0; overrideCount < 10; overrideCount++) {
        ...
        domInfo = this.getModelDOM(overrideModel || model, subtemplatePath) || {dom:'<div/>',domModels:[overrideModel || model]};
        if ((!(jqnode = $(domInfo.dom))) || (jqnode.length !== 1)) { jqnode = $('<div/>'); }
        jqnode[0].id = elid;

        if (jqnode[0].hasAttribute('overridevariant')) {
          const overrideVariant = this.quickTemplatedText(jqnode[0].getAttribute('overridevariant'),model,fieldSubstitutes,elid);
          if (overrideVariant!=='<default>') { newOverrideModel = this.model(this.modelIdWithVariant(model.id,overrideVariant)); }
        } else if (jqnode[0].hasAttribute('overridemodelid')) {
          const overrideModelId = this.quickTemplatedText(jqnode[0].getAttribute('overridemodelid'),model,fieldSubstitutes,elid);
          if (overrideModelId!=='<default>') { newOverrideModel = this.model(overrideModelId); }
        } else {
          break;
        }

        if (newOverrideModel === (overrideModel || model)) { break; }
      ...
      if (overrideModel) { jqnode[0].setAttribute("overridemodelid",overrideModel.id); }
      ...
      if (overrideModel) { useModel = overrideModel; }
      ...
      if (jqwrapped[0]!==jqnode[0]) { this.queueModelEvent(jqwrapped[0],'insertmodel'); }

      if (useModel) { this.insertNewModelElementChildren(useModel,jqnode,justReturnMapping,depth,inSubtemplateForModelIds,ret); }
      if (useParentModel && (useModel!==useParentModel)) { this.insertNewModelElementChildren(useParentModel,jqnode,justReturnMapping,depth,inSubtemplateForModelIds,ret); }
    ...
    insertNewModelElementChildren(model,jqnode,justReturnMapping,depth,inSubtemplateForModelIds,ret){
      ...
      for (var childField in model.fields) {
        ...
        if ((childField !== 'subtemplates') && f.array) {var m;
          ...
          if (!(inSubtemplateForModelIds && inSubtemplateForModelIds[model.id])) {
            ...
            if (!jqnode.is(childMarkerSel)) {
              for (childMarkerEl of jqnode.find(childMarkerSel)) {
                ...
                if (mappings = this.insertNewModelElement(model,jqprevChildEl,childMarkerEl.getAttribute("__collectionindex"),jqpar,model,undefined,undefined,childField,markerClass,false,depth,inSubtemplateForModelIds)) {
                  jqprevChildEl = mappings[0].add;
                }
    ...
    setupModelNodeSubtree(model,parentModel,usingParentModel,jqnode,isDesc,field,elid,acollectionIndex,fieldSubstitutes,template,subtemplatePath,modelChildIndexesByField){
      ...
      if ((jqnode.length!==1) || (isDesc && jqnode.hasClass('modelStop'))) { return; }
      const node = jqnode[0];
          ...
          if (match) {
            ...
            if (node.hasAttribute("fieldasmodelchild")) {
              ...
              if (startupFields = this.startupTemplateFields(field)) {
                field = this.templatedText(field,fieldSubstitutes||{},elid,usingModel.id,parentModel,startupFields);
        ...
        // setup model subtemplates, any node marked with a class like users-subtemplate will be seen as a marker for an array of children under that field, the difference being that they are the children of the template
          ...
          if (match) {
            ...
            if (node.hasAttribute('model')) {
              ...
              markerClass = model.classAsChild+"__"+this.markerIndex('subtemplates')+'__'+this.sanitizeClassName(newPath,true);
              collectionIndex = acollectionIndex[0]++;
              if (!jqnode.find(".model-placeholder").length) {
                addClasses["model-placeholder"] = true;
                addClasses[markerClass+"_placeholder"] = true;
      ...
      if (isPlaceholder) {
          ...
          for (let attr of node.attributes) {
            if (attr.specified) {
              ...
              if (startupFields = this.startupTemplateFields(value)) {
                value = this.templatedText(value,fieldSubstitutes||{},elid,usingModel.id,parentModel,startupFields);
        ...
        for (let child of node.childNodes) {
          if (child.nodeType === 3) { // text node
            ...
            this.setupModelNodeSubtree(model,parentModel,usingParentModel,$(child),true,field,elid,acollectionIndex,fieldSubstitutes,template,subtemplatePath,modelChildIndexesByField);
          }
        }
      }
      ...
      for (let k in addAttr) { const v = addAttr[k]; node.setAttribute(k,v); }
      for (cls in addClasses) { jqnode.addClass(cls); }
    ...
    overrideVariant(el,variant){
      ...
      if ((!(jqel = this.modelElementForElement(el))) || (!(model = this.modelForElement(jqel[0]))) || (!(overrideModelId = this.modelIdWithVariant(model.id,variant)))) { return false; }
    ...
    overrideModelId(el,overrideModelId){
      ...
      if ((!(jqel = this.modelElementForElement(el))) || (!(model = this.modelForElement(jqel[0])))) { return false; }
      el = jqel[0];

      if (el.hasAttribute('overridemodelid')) { overrideModelIdWas = el.getAttribute('overridemodelid'); }
      if (overrideModelId===overrideModelIdWas) { return false; }
      ...
      if (!Object.getOwnPropertyNames(this.needModels).length) { this.updateModels(); }
      return false;
    }
  };
