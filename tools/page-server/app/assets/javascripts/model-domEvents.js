...
document.ModelDOM_classes.ModelDOM_domEvents = ModelDOM_domEvents = class ModelDOM_domEvents {
  getTargetIfEvent(eventOrElement, cancelBubble, preventDefault) {
    if (eventOrElement instanceof jQuery.Event || eventOrElement instanceof Event) {
      ...
      if (event.cancelBubble || (event.originalEvent && event.originalEvent.cancelBubble)) {
        return false;
      }
...
$(document).ready(function() {
  $(window).focus(function() {
    if (document.modelDOM.windowIsFocussed) {
      return;
    }
  ...
  $(document).on("click", ".model-anchor[href]", function(event) {
    ...
    return $(`a[name="${link}"]`).each(function() {
      let el;
      if (
        (el = document.modelDOM.modelElementForElement(this)).length &&
        el[0].parentElement &&
        (el = document.modelDOM.modelElementForElement(el[0].parentElement)).length
      ) {
        el.addClass("flash-heart-beat");
        return setTimeout(() => el.removeClass("flash-heart-beat"), 2000);
      }
    });
  });
  ...
  $(document).on("dragstart", ".model-draggable", function(event) {
    ...
    if (!event.originalEvent || !(transfer = event.originalEvent.dataTransfer)) {
      return;
    }
    if (
      !(id = document.modelDOM.modelIdForElement(event.target)) ||
      !(context = event.target.getAttribute("dragcontextmodelid")) ||
      !(field = event.target.getAttribute("dragcontextfield"))
    ) {
  ...
  $(document).on("dragenter", ".model-drag-space", function(event) {
    if (!event.originalEvent || !event.originalEvent.fromElement) {
      return;
    }
    if (
      event.originalEvent.fromElement.getAttribute("dragcontextmodelid") !==
      event.target.getAttribute("dragcontextmodelid")
    ) {
      return;
    }
    if (
      event.originalEvent.fromElement.getAttribute("dragcontextfield") !== event.target.getAttribute("dragcontextfield")
    ) {
      return;
    }
  ...
  return $(document).on("drop", ".model-drag-space", function(event) {
    ...
    if (!(transfer = event.originalEvent.dataTransfer)) {
      return;
    }
    if (
      !(context = transfer.getData("contextmodelid")) ||
      !(field = transfer.getData("contextfield")) ||
      !(id = transfer.getData("itemmodelid"))
    ) {
      return;
    }
    if (context !== event.target.getAttribute("dragcontextmodelid")) {
      return;
    }
    if (field !== event.target.getAttribute("dragcontextfield")) {
      return;
    }
