...
document.ModelDOM_classes.ModelDOM_modelEvents = (ModelDOM_modelEvents = class ModelDOM_modelEvents {
  constructor() {
    ...
    this.defaultModelCallbacks={
      insertmodel() {
        let fadeIn, jqel;
        if (!(jqel = me.modelElementForElement(this))) { return; }
        const el = jqel[0];
  ...
  queueModelEvent(el,type, ifElementHasHandler){
    let cb;
    if (ifElementHasHandler && (!(cb = element.getAttribute('onchangemodel')) || (typeof(cb)!=='string') || !this.getFunction(cb))) { return; }

    const { modelEvents } = this;
    if (!modelEvents[type]) { modelEvents[type] = []; }
  ...
  getFunction(fn){
    ...
    if (!this._functionLookup) { this._functionLookup = {}; }
    if ((ret = this._functionLookup[fn]) !== undefined) {
      if (typeof(ret)==='function') { return ret; }
    } else {
      //try TODO
      ret = Function(fn);
      if (typeof(ret)!=='function') { ret = null; }
    ...
    for (let type of ['removemodel', 'insertmodel', 'changemodel']) {
      if ((elements = modelEvents[type])) {
        for (let element of Array.from(elements)) {
          ...
          if (type==='removemodel') {
            if (cbret!==false) { cbret = 0; }
            if (typeof(cbret)==='number') {
              if (!delayedRemoves[cbret]) { delayedRemoves[cbret] = []; }
