...
document.ModelDOM_classes.ModelDOM_elements = ModelDOM_elements = class ModelDOM_elements {
  ...
  modelIdForElement(el, bubblesToParent, checkThisElementFirst, ignoreOverride) {
    ...
    let root;
    if ($(el).hasClass("__deadModel")) {
      return;
    }

    if (checkThisElementFirst && el) {
      if (bubblesToParent || el.getAttribute("using-parent-model")) {
        if (el.hasAttribute("parentmodelid")) {
          return el.getAttribute("parentmodelid");
        }
    ...
    if (!(root = this.modelElementForElement(el))) {
      return;
    }
    if (bubblesToParent || el.getAttribute("using-parent-model")) {
  ...
  markerElementForElement(el) {
    ...
    if (!(jqel = this.modelElementForElement(el))) {
      return;
    }
    if (!(collectionIndex = (el = jqel[0]).getAttribute("__collectionindex"))) {
      return;
    }
    if (!(markerClass = el.getAttribute("__markerClass"))) {
      return;
    }
    const markerCollectionClass = markerClass + "_collection-" + collectionIndex;

    if ((jqmarker = jqel.parent().children(`.${markerCollectionClass}`)).length !== 1) {
      return;
    }
    return jqmarker;
  }
  ...
  modelForElement(el, bubblesToParent, checkThisElementFirst, ignoreOverride) {
    ...
    let id;
    if ((id = this.modelIdForElement(el, bubblesToParent, checkThisElementFirst, ignoreOverride)) !== undefined) {
