...
document.ModelDOM_classes.ModelDOM_util = (ModelDOM_util = class ModelDOM_util {
  ...
  isEqual(a,b){
    ...
    if ($.isPlainObject(a)) {
      ...
      for (var k of Object.keys(a || {})) {
        v = a[k];
        if (!b.hasOwnProperty[k] || !document.modelDOM.isEqual(v,b[k])) { return false; }
      }
      ...
      for (let i = 0; i < a.length; i++) {
        v = a[i];
        if (!document.modelDOM.isEqual(v,b[i])) { return false; }
      }
      return true;
    } else {
      return a===b;
  ...
  sanitizeClassName(name,ignoreClassStartRules){
    ...
    while ((match = re.exec(name)) !== null) {
      if (match.index>index) { ret += name.substring(index,match.index); }
      ret += name.charCodeAt(match.index)+"-";
      index = match.index+1;
    }
    if (name.length>index) { ret += name.substring(index); }

    if (!ignoreClassStartRules && ((ret.length<2) || /^\d/.test(ret))) { ret = `c-${ret}`; }
    return ret;
  }
