...
document.ModelDOM_classes.ModelDOM_diff = ModelDOM_diff = class ModelDOM_diff {
    ...
    for (field in diff) {
      ...
      if ($.isArray(d)) {
        ...
        if ($.isPlainObject(f) && f.array) {
          ...
          for (childInfo of f.array) {
            if ($.isPlainObject(childInfo) && childInfo.model) {
              this.unlinkModels(model, childInfo.model);
            }
          }
        }
    ...
    if (!isEdit) {
      const removeFields = (() => {
        ...
        for (field in model.fields) {
          if (
            !diff.hasOwnProperty(field) ||
            (f = diff[field]) === undefined ||
            f === null ||
            (typeof f === "number" && Math.abs(f) < this.JSON_model_zero_detect_tolerance)
          ) {
            result1.push(field);
          }
        }
      ...
      for (field of removeFields) {
        ...
        if ($.isPlainObject(f) && f.array) {
          model.fieldChanges[field] = (() => {
            const result2 = [];
            for (v of f.array) {
              result2.push(v.index);
            }
            return result2;
          })();
          for (childInfo of f.array) {
            if (childInfo && childInfo.model) {
              this.unlinkModels(model, childInfo.model);
            }
          }
        } else {
  ...
  applyModelArrayDiff(array, diff, parentModel, field) {
    ...
    if (!$.isPlainObject(array) || !$.isArray(array.array) || !$.isPlainObject(array.changes)) {
      array = { array: [], changes: {}, markerIndex: this.markerIndex(field) };
    }
    ...
    for (let i = 0; i < diff.length; i++) {
      ...
      if (
        typeof v === "number" &&
        v >= -this.JSON_model_max_count - this.JSON_model_int_detect_tolerance &&
        v <= this.JSON_model_max_count + this.JSON_model_int_detect_tolerance &&
        Math.abs(v - Math.round(v)) < this.JSON_model_int_detect_tolerance
      ) {
        v = Math.round(v);
        if (v === 0) {
          ...
          if (dels > 0) {
            var asc, end;
            for (
              mi = modelInd, end = modelInd + dels, asc = modelInd <= end;
              asc ? mi < end : mi > end;
              asc ? mi++ : mi--
            ) {
              m = array.array[mi];
              if (m.model) {
                this.unlinkModels(parentModel, m.model);
              }
          ...
          if ((skip = Math.min(array.array.length - modelInd, v))) {
            ...
            if ((c = array.changes[array.array[modelInd].index]) && c.prevIndex) {
        ...
        if (modelInd < array.array.length) {
          ...
          if (m.model !== v) {
            ...
            m.model = v;
            if (m.model) {
              this.linkModels(parentModel, m.model);
            }
            ...
            if ((c = array.changes[m.index]) && c.prevIndex) {
              c.prevIndex = prevIndex;
            }
          }
        } else {
    ...
    if (modelInd < array.array.length) {
      ...
      let asc1, end1;
      for (
        mi = modelInd, end1 = array.array.length, asc1 = modelInd <= end1;
        asc1 ? mi < end1 : mi > end1;
        asc1 ? mi++ : mi--
      ) {
        m = array.array[mi];
        if (m.model) {
  ...
  applyArrayDiff(array, diff) {
    ...
    for (let i = 0; i < diff.length; i++) {
      ...
      if (
        ...
        if (
          typeof v === "number" &&
          v >= -this.JSON_model_max_count - this.JSON_model_int_correct_tolerance &&
          v <= this.JSON_model_max_count + this.JSON_model_int_correct_tolerance &&
          Math.abs(v - Math.round(v)) < this.JSON_model_int_correct_tolerance
        ) {
          v = Math.round(v);
        }
  ...
  clearModelChanges() {
    ...
    for (let id in this.changedModels) {
      ...
      for (let field in model.fieldChanges) {
        if (
          $.isPlainObject(model.fields[field]) &&
          $.isPlainObject(model.fields[field].changes) &&
          $.isArray(model.fields[field].array)
        ) {
          model.fields[field].changes = {};
        }
      }
      model.fieldChanges = {};
    }
