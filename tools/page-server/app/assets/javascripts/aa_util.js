...
window.ERROR = function() {
  console.log(
    "==========Ahem, terribly sorry old chap, there appears to be a small issue that probably warrants brief inquiry:"
  );
...
window.WARN = function() {
  console.log(
    "==========Ahem, terribly sorry old chap, there appears to be a little problem. I'll do my best to carry on."
  );
...
Array.newArrayWithSize = function(size, meOrValue, fn) {
  ...
  if (this.standard.length < size) {
    for (
      let add = this.standard.length, end = size, asc = this.standard.length <= end;
      asc ? add < end : add > end;
      asc ? add++ : add--
    ) {
...
Array.prototype.copyWithSize = function(size, meOrValue, fn) {
  if (this.length >= size) {
    ...
    const { length } = this;
    return this.concat(
      Array.newArrayWithSize(
        size - length,
        meOrValue,
        typeof fn === "function" ? i => fn.call(meOrValue, i + length) : fn
      )
    );
  }
};
  ...
  document.aautil = {
    ...
    _deferableLooseCallbackFired(name) {
      ...
      if ((cb = document.aautil._deferableLooseCallbacks[name])) {
        ...
        if ((msec = cb[0]) > 0) {
          cb[0] = 0;
          return setTimeout(() => document.aautil._deferableLooseCallbackFired(name), msec);
        } else {
          cb[1]();
    ...
    goFullscreen: el => {
      let jqel, ret;
      if (typeof el !== "object" && (typeof el !== "string" || !(jqel = $(el)).length || !(el = jqel[0]))) {
        return false;
      }
      return (ret = (() => {
    ...
    killTopModal(el, count) {
      ...
      if (!count || !(modal = document.aautil._modals.pop())) {
        return;
      }
      if (el && modal.jqel[0] === el) {
        count = 1;
      }

      if (modal.jqbg) {
        modal.jqbg.removeClass("displayed");
      }
    ...
    startModal(el, grpel, callback) {
      ...
      if ((l = document.aautil._modals.length)) {
        ...
        setTimeout(() => jqbg.addClass("displayed"), 50);
      }
    ...
    toggle(el, event, setOffsToOn, setOnsToOff) {
      ...
      if (groupName) {
        if (groupName) {
          groupSel = `[toggle="${groupName}"]`;
        }
        if (groupSel) {
          group = $(el).closest(`.toggle-group${groupSel}`);
        }
        if (!group || !group.length) {
          group = $(el).closest(".toggle-group:not([toggle])");
        }
        if (!group || !group.length) {
          group = $(el).parent();
        }
      ...
      if (group.hasClass("toggle-modal")) {
        let focus;
        if (groupSel) {
          focus = group.find(`.toggle-focus${groupSel}`);
        }
        if (!focus || !focus.length) {
          focus = group.find(".toggle-focus:not([toggle])");
        }
        if (!focus.length) {
          focus = group;
        }
      ...
      if (setOffsToOn) {
        ...
        setTimeout(() => togoff.addClass("toggle-on"), 50);
      }
      ...
      if (setOnsToOff) {
        ...
        togon.filter(".toggle-display,.toggle-fader").each(function() {
          let delay;
          const jqel = $(this);
          if ((delay = this.getAttribute("delay")) <= 0) {
            delay = 400;
          }
          return setTimeout(function() {
            if (jqel.hasClass("toggle-on")) {
              return;
            }
    ...
    stylesheet() {
      if (document.aautil._stylesheet) {
        return document.aautil._stylesheet;
      }
    ...
    addStylesheetRule(rule, name, atIndex) {
      ...
      const stylesheet = document.aautil.stylesheet();
      if ((index = document.aautil._stylesheetRuleNames.indexOf(name)) >= 0) {
        if (atIndex === undefined && document.aautil._stylesheetRules[index] === rule) {
          return;
        }
      ...
      if (atIndex === undefined) {
        atIndex = document.aautil._stylesheetRuleNames.length;
      }
    ...
    scrollbarWidth() {
      if (document.aautil._scrollbarWidth !== undefined) {
        return document.aautil._scrollbarWidth;
      }
...
$(document).ready(function() {
  ...
  return document.aautil.addStylesheetRule(
    `body.has-scrollbar.no-scroll{padding-right:${scrollbarWidth}px;}`,
    "body-steady-scrollbar-right"
  );
});
...
$(document).on("input", "input[sizer]", function(event) {
  let jqsizer;
  if (!(jqsizer = $(`#${event.target.getAttribute("sizer")}`)).length) {
    return;
  }
...
$(document).on("keydown", ".posting-text", function(event) {
  ...
  if (event.keyCode === 13) {
    // newline
    if (el.getAttribute("lines") !== "single" && !!event.shiftKey) {
      return;
    }
    event.preventDefault();
    event.stopPropagation();
    if (!el.hasAttribute("model") || !el.hasAttribute("name")) {
      return;
    }
