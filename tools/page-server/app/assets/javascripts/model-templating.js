...
document.ModelDOM_classes.ModelDOM_templating = ModelDOM_templating = class ModelDOM_templating {
                ...
                function(v) {
                  if (
                    ...
                    (v === true ||
                      typeof v === "string" ||
                      typeof v === "number" ||
                      ($.isPlainObject(v) && v.array && v.array.length))
                  ) {
                    return val;
                  }
                })(val);
            case "->":
              return (val => v => val)(val);
            case "*":
              return (mul =>
                function(v) {
  ...
  templatedText(text, dict, elid, modelId, parentModelId, fields, noDefaults, escape) {
    ...
    if (!fields && !(fields = this.templateFields(text))) {
      return text;
    }
    ...
    for (var field of fields) {
      if (field.start > ind) {
        ret += text.substring(ind, field.start);
      }
      ...
      ret += (() => {
        ...
        switch (field.field) {
          ...
          case "__baseparentmodel":
            if (parentModelId !== undefined) {
              return this.modelIdWithVariant(typeof parentModelId === "string" ? parentModelId : parentModelId.id);
            } else if (noDefaults && typeof (v = dict[field.field]) === "string") {
              return v;
            } else {
              return text.substring(field.start, field.start + field.length);
    ...
    if (ind < text.length) {
      ret += text.substring(ind, text.length);
    }
