  ...
  await TestRig.go(
    ...
    async function(rig) {
      ...
      let datapoint,
        ...
        datapointIds = [
          simpleDatapointId,
          dpWithDependentWithDependentId,
          dpWithGetterAndDependencyAndDependentId,
          dpWithGetterAndDependencyWithDependencyId,
          dpWithDependentId,
          dpWithGetterAndDependencyId,
          dpWithGetterId,
          dpWithGetterUsingParentId,
        ],
        datapointsWithGetterIds = [
          dpWithGetterAndDependencyAndDependentId,
          dpWithGetterAndDependencyWithDependencyId,
          dpWithGetterAndDependencyId,
          dpWithGetterId,
        ...
        datapointsWithDependencyIds = [
          dpWithGetterAndDependencyAndDependentId,
          dpWithGetterAndDependencyWithDependencyId,
          dpWithGetterAndDependencyId,
        ],
        datapointsWithDependentIds = [
          dpWithDependentWithDependentId,
          dpWithGetterAndDependencyAndDependentId,
          dpWithDependentId,
        ],
        datapointsWithoutDependentIds = [
          simpleDatapointId,
          dpWithGetterAndDependencyWithDependencyId,
          dpWithGetterAndDependencyId,
          dpWithGetterId,
        ...
        datapointsWithoutDependencyIds = [
          simpleDatapointId,
          dpWithDependentWithDependentId,
          dpWithDependentId,
          dpWithGetterId,
      ...
      //         DD2: callbackKey not specified
      await rig.assert(
        `a watch was started on datapoint ${datapointId} with listeners and without a callbackKey specified`,
        (callbackKeys[datapointId] = datapoints[datapointId].watch({
          onvalid: datapoint =>
            (onvalidCalls[datapointId] = {
      ...
      for (const datapointId of datapointsWithoutDependentIds) {
        await rig.assert(
          `a watch was started on datapoint ${datapointId} with a callbackKey specified and without listeners`,
      ...
      //         DB
      await rig.assert(
        `the datapoint ${datapointId} is valid and its value is correctly returned by valueIfAny`,
        datapoints[datapointId].valueIfAny,
        {
          equals: expectedValues[datapointId],
        }
      );
      ...
      for (const datapointId of datapointIds) {
        await rig.assert(
          `the value of already valid datapoint ${datapointId} is correctly returned by value`,
      ...
      //         DE1: was not last listener
      await rig.assert(
        `the first watch was stopped on datapoint ${datapointId}`,
        await datapoints[datapointId].stopWatching({
      ...
      for (const datapointId of datapointsWithoutDependentIds) {
        await rig.assert(
          `the last watch was stopped on datapoint ${datapointId}`,
          datapoints[datapointId].stopWatching({
            callbackKey: baseCallbackKey,
          }),
          {
            includes: {
      ...
      //         DF1: was already invalid
      await rig.assert(
        `invalidate is idempotent on already invalid datapoint ${datapointId}`,
      ...
      for (const datapointId of datapointIds) {
        await rig.assert(
          `the value of recreated datapoint ${datapointId} is correct`,
          await datapoints[datapointId].value,
          {
            equals: expectedValues[datapointId],
          }
      ...
      for (const datapointId of datapointsWithoutDependencyIds) {
        await rig.assert(
          `invalidate clears value on datapoint ${datapointId}`,
          datapoints[datapointId].invalidate({ queueValidationJob: false }),
          {
            includes: {
              valueIfAny: false,
      ...
      for (const datapointId of datapointsWithDependencyIds) {
        await rig.assert(
          `invalidate of dependency cleared value on datapoint ${datapointId}`,
      ...
      await rig.assert(
        `invalidate of datapoint ${dpWithDependentWithDependentId} that has an invalid dependent clears value`,
      ...
      await rig.assert(
        `invalidate of datapoint ${dpWithDependentId} that has an invalid dependent clears value`,
      ...
      //         CD
      await rig.assert(
        `commitNewlyUpdatedDatapoints doesn't throw`,
        await datapointCache.commitNewlyUpdatedDatapoints(),
        {
          throws: false,
        }
      );
      ...
      await rig.assert(
        `the new value of datapoint ${datapointId} is correctly read back`,
        await datapoints[datapointId].value,
        {
      ...
      await rig.assert(
        `the new value of datapoint ${dpWithGetterUsingParentId} is correctly read back`,
        await datapoints[dpWithGetterUsingParentId].value,
        {
